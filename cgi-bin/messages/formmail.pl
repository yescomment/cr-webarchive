...
@referers = ('cityreliquary.org');

# @recipients defines the e-mail addresses or domain names that e-mail can   #
# be sent to.  This must be filled in correctly to prevent SPAM and allow    #
# valid addresses to receive e-mail.  Read the documentation to find out how #
# this variable works!!!  It is EXTREMELY IMPORTANT.                         #
@recipients = &fill_recipients('cityreliquary.org', @referers);
...
sub send_mail {
    ...
    if (!$Config{'email'}) { $Config{'email'} = "postmaster\@cityreliquary.org"; }
    if (!$Config{'recipient'}) { $Config{'recipient'} = "postmaster\@cityreliquary.org"; }
