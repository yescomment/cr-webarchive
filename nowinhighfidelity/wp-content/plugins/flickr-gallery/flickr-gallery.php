...
if ( defined('CODEMEME_TEST') ) {
	...
	define('CM_FLICKR_GALLERY_VERSION', '1.2.7');
}
...
class DC_FlickrGallery {
    ...
    function init() {
		...
		if ( get_option('fg-API-key') ) {
			...
			if ( DC_FlickrGallery::get_major_version() >= 2.8 ) {
				wp_enqueue_style('fg-jquery-ui', DC_FlickrGallery::getURL() . 'tab-theme/jquery-ui-1.7.1.css', array(), '1.7.1');			
			} else {
				...
				default :
					if ( !empty($attr['user_id']) ) {
						...
						$tabs = apply_filters('flickr_gallery_tabs', array(
							array('id'=>'photostream', 'name'=>'Photostream'),
							array('id'=>'sets', 'name'=>'Photosets'),
							array('id'=>'interesting', 'name'=>'Most Interesting'),
						));
						$tab_count = 0;
						?>
													...
													$("#photostream-next a, #photostream-prev a").click(function(e){
														...
														$.post("<?php echo $_SERVER['REQUEST_URI'] ?>", {
															...
															<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
																$("#photostream .flickr-thumb img").flightbox();
															<?php endif; ?>
															if ( rsp.page == 1 ) {
											...
											(function($){
												$(document).ready(function(){
													function flickr_gallery_load_photoset($display, id, page) {
														...
														$display.load("<?php echo get_bloginfo('wpurl') ?>?action=flickr-gallery-photoset&pagination=<?php echo $attr['pagination'] ?>&page=" + page + "&id=" + id, null, function(){
															<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
																$display.find(".flickr-thumb img").flightbox();
															<?php endif; ?>
												...
												$(document).ready(function(){
													$("#interesting-next a, #interesting-prev a").click(function(e){
														...
														$.post("<?php echo $_SERVER['REQUEST_URI'] ?>", {
															...
															<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
																$("#interesting .flickr-thumb img").flightbox();
															<?php endif; ?>
															if ( rsp.page == 1 ) {
							...
							<script type="text/javascript">
								jQuery(document).ready(function(){
									...
									<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
										jQuery("#gallery-<?php echo $id ?> .flickr-thumb img").flightbox();
									<?php endif; ?>
								});
							</script>
						<?php
							...
							$(document).ready(function(){
								$("#fg-<?php echo $id ?>-next a, #fg-<?php echo $id ?>-prev a").click(function(e){
									...
									$.post("<?php echo $_SERVER['REQUEST_URI'] ?>", {
										...
										<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
											$("#gallery-<?php echo $id ?> .flickr-thumb img").flightbox();
										<?php endif; ?>
										if ( rsp.page == 1 ) {
		...
		?>
			<div class="wrap">
				...
				<form method="post" action="admin-ajax.php" id="flickr-gallery-settings">
					...
					<input type="hidden" name="page_options" value="fg-API-key,fg-secret,fg-token,fg-user_id,fg-per_page,fg-db-cache,fg-credit-link,fg-flightbox" />
					<p>
					...
					<p><strong><?php _e("If you want the plugin to include private photos or photos in private groups, you will need to fill in the following fields.  Leave them blank otherwise.  Also, you will need to make sure that your API key's authentication type is set to Desktop at Flickr.", 'flickr-gallery') ?></strong></p>
					<p>
					...
					<p>
						<?php if ( get_option('fg-secret') && get_option('fg-api-key') ) : ?>
							<?php printf(__('Authentication Token (<a target="_blank" id="auth-link" href="%s">generate</a>):', 'flickr-gallery'), $phpFlickr->desktop_auth()) ?>
							<input type="hidden" name="fg-frob" id="fg-frob" value="<?php echo $phpFlickr->parsed_response['frob'] ?>" />
						<?php else : ?>
					...
					<p>
						...
						<input id="fg-credit-link" type="checkbox" value="1" name="fg-credit-link" <?php echo get_option('fg-credit-link') || get_option('fg-credit-link')===false ? 'checked="checked"' : '' ?> />
					</p>
	...
	function ajax_pagination() {
		...
		echo '{page:' . $_POST['page'] . ',pages:' . $pager->pages . ',html:"' . str_replace('"', '\\"', str_replace('\\', '\\\\', $html)) . '"}';
		exit;
	}
...
add_action('wp_ajax_flickr_gallery_auth', array('DC_FlickrGallery', 'get_auth_token'));
