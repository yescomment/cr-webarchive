...
if ( !class_exists('phpFlickr') ) {
	...
	/*
		...
		function desktop_auth ($perms = "read") {
			$frob = $this->auth_getFrob();
			$api_sig = md5($this->secret . 'api_key' . $this->api_key . 'frob' . $frob . 'perms' . $perms);
			return 'http://flickr.com/services/auth/?api_key=' . $this->api_key . '&frob=' . $frob . '&perms=' . $perms . '&api_sig=' . $api_sig;
		}
		...
		*/
		...
		function auth_getFrob ()
		{
			...
			$this->request('flickr.auth.getFrob', array(), true);
